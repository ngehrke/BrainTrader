...
public class ApiController implements EWrapper {
	private com.ib.controller.ApiConnection m_client;
	...
	public com.ib.controller.ApiConnection client() { return m_client; }
	...
	public ApiController( IConnectionHandler handler, ILogger inLogger, ILogger outLogger) {
		...
		m_client = new com.ib.controller.ApiConnection( this, inLogger, outLogger);
		m_inLogger = inLogger;
	...
	public interface IAccountHandler {
		...
		void updatePortfolio(com.ib.controller.Position position);
	}
	...
	@Override public void updatePortfolio(Contract contract, Decimal positionIn, double marketPrice, double marketValue, double averageCost, double unrealizedPNL, double realizedPNL, String account) {
		...
		com.ib.controller.Position position = new com.ib.controller.Position( contract, account, positionIn, marketPrice, marketValue, averageCost, unrealizedPNL, realizedPNL);
		for( IAccountHandler handler : m_accountHandlers) {
	...
	public interface IAdvisorHandler {
		void groups(List<com.ib.controller.Group> groups);
		void aliases(List<com.ib.controller.Alias> aliases);
	...
	public void updateGroups( List<com.ib.controller.Group> groups) {
		if (!checkConnection())
			return;

		m_client.replaceFA( REPLACE_FA_GROUPS_REQ_ID, FADataType.GROUPS.id(), com.ib.controller.AdvisorUtil.getGroupsXml( groups) );
		sendEOM();
	}
	...
	@Override public final void receiveFA(int faDataType, String xml) {
		...
		switch( type) {
			case FADataType.GROUPS:
				List<com.ib.controller.Group> groups = com.ib.controller.AdvisorUtil.getGroups( xml);
				m_advisorHandler.groups(groups);
				break;

			case FADataType.ALIASES:
				List<com.ib.controller.Alias> aliases = com.ib.controller.AdvisorUtil.getAliases( xml);
	...
	public interface IHistoricalDataHandler {
		void historicalData(com.ib.controller.Bar bar);
	...
	@Override public void historicalData(int reqId, com.ib.client.Bar bar) {
		...
		if (handler != null) {
			if (bar.time().startsWith( "finished")) {
				...
				com.ib.controller.Bar bar2 = new com.ib.controller.Bar( bar.time(), bar.high(), bar.low(), bar.open(), bar.close(), bar.wap(), bar.volume(), bar.count());
	...
	public interface IRealTimeBarHandler {
		void realtimeBar(com.ib.controller.Bar bar); // time is in seconds since epoch
	}
    ...
    @Override public void realtimeBar(int reqId, long time, double open, double high, double low, double close, Decimal volume, Decimal wap, int count) {
		...
		if (handler != null) {
			com.ib.controller.Bar bar = new com.ib.controller.Bar( time, high, low, open, close, wap, volume, count);
