...
class EDecoder implements ObjectInput {
    ...
    private void processHistoricalTicksLast() throws IOException {
        ...
        for (int i = 0; i < tickCount; i++) {
            long time = readLong();
            com.ib.client.BitMask mask = new com.ib.client.BitMask(readInt());
            ...
            com.ib.client.Decimal size = readDecimal();
    ...
    private void processHistoricalTicksBidAsk() throws IOException {
        ...
        for (int i = 0; i < tickCount; i++) {
            ...
            double priceBid = readDouble(),
                   priceAsk = readDouble();
            com.ib.client.Decimal sizeBid = readDecimal(),
                    sizeAsk = readDecimal();
    ...
    private void processHistoricalDataUpdateMsg() throws IOException {
        ...
        com.ib.client.Decimal WAP = readDecimal();
        com.ib.client.Decimal volume = readDecimal();

        m_EWrapper.historicalDataUpdate(reqId, new com.ib.client.Bar(date, open, high, low, close, volume, barCount, WAP));
    }

    private void processPnLSingleMsg() throws IOException {
    	int reqId = readInt();
    	com.ib.client.Decimal pos = readDecimal();
    	double dailyPnL = readDouble();
    	double unrealizedPnL = Double.MAX_VALUE;
        double realizedPnL = Double.MAX_VALUE;
    	
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_UNREALIZED_PNL) {
            unrealizedPnL = readDouble();
        }

        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_REALIZED_PNL) {
    ...
    private void processMktDepthExchangesMsg() throws IOException {
        ...
        if (nDepthMktDataDescriptions > 0) {
            ...
            {
                if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_SERVICE_DATA_TYPE) {
    ...
    private void processSymbolSamplesMsg() throws IOException {
        ...
        if (nContractDescriptions > 0){
            ...
            for (int i = 0; i < nContractDescriptions; i++)
            {
                ...
                if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_BOND_ISSUERID) {
	...
	private void processHistoricalDataMsg() throws IOException {
	    ...
	    if (m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_SYNT_REALTIME_BARS) {
	        version = readInt();
	    }
	...
	private void processMarketDepthL2Msg() throws IOException {
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_SMART_DEPTH) {
	...
	private void processExecutionDataMsg() throws IOException {
		...
		if (m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_LAST_LIQUIDITY) {
		    version = readInt();
		}
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_MODELS_SUPPORT) {
        ...
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PENDING_PRICE_REVISION) {
	...
	private void processBondContractDataMsg() throws IOException {
		int version = 6;
		if (m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_SIZE_RULES) {
			version = readInt();
		}
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_MD_SIZE_MULTIPLIER && m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_SIZE_RULES) {
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_AGG_GROUP) {
			contract.aggGroup(readInt());
		}
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_MARKET_RULES) {
	...
	private void processContractDataMsg() throws IOException {
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_LAST_TRADE_DATE) {
		...
		if( version >= 5) {
		   contract.longName(m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_ENCODE_MSG_ASCII7 ? decodeUnicodeEscapedString(readStr()) : readStr());
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_UNDERLYING_INFO) {
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_REAL_EXPIRATION_DATE) {
			contract.realExpirationDate(readStr());
		}
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_STOCK_TYPE) {
		    contract.stockType(readStr());
		}
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_FRACTIONAL_SIZE_SUPPORT && m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_SIZE_RULES) {
        ...
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_FUND_DATA_FIELDS && contract.contract().secType() == SecType.FUND) {
        ...
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_INELIGIBILITY_REASONS) {
    ...
    private void processOpenOrderMsg() throws IOException {
        ...
        // read version
        int version = m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_ORDER_CONTAINER ? readInt() : m_serverVersion;
        ...
        eOrderDecoder.readAutoCancelParent(com.ib.client.EClient.MIN_SERVER_VER_AUTO_CANCEL_PARENT);
	...
	private void processErrMsgMsg() throws IOException {
		...
		if(version < 2) {
		    ...
		    String errorMsg = m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_ENCODE_MSG_ASCII7 ? decodeUnicodeEscapedString(readStr()) : readStr();
		    String advancedOrderRejectJson = null;
		    if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_ADVANCED_ORDER_REJECT) {
	...
	private void processPortfolioValueMsg() throws IOException {
		...
		com.ib.client.Decimal position = readDecimal();
	...
	private void processOrderStatusMsg() throws IOException {
		int version = m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_MARKET_CAP_PRICE ? Integer.MAX_VALUE : readInt();
		int id = readInt();
		String status = readStr();
		com.ib.client.Decimal filled = readDecimal();
		com.ib.client.Decimal remaining = readDecimal();
		...
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_MARKET_CAP_PRICE) {
	...
	private void processTickOptionComputationMsg() throws IOException {
		int version = m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PRICE_BASED_VOLATILITY ? Integer.MAX_VALUE : readInt();
		int tickerId = readInt();
		int tickType = readInt();
		int tickAttrib = Integer.MAX_VALUE;
		if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PRICE_BASED_VOLATILITY) {
			tickAttrib = readInt();
		}
	...
	private void processTickPriceMsg() throws IOException {
		...
		int tickType = readInt();
		double price = readDouble();
		com.ib.client.Decimal size = com.ib.client.Decimal.INVALID;
		...
		if (version >= 3) {		
			...
			if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PAST_LIMIT) {
				com.ib.client.BitMask mask = new com.ib.client.BitMask(attrMask);
    ...
    private void processTickByTickMsg() throws IOException {
        ...
        switch(tickType){
            ...
            case 2: // AllLast
                double price = readDouble();
                com.ib.client.Decimal size = readDecimal();
                mask = new com.ib.client.BitMask(readInt());
                TickAttribLast tickAttribLast = new TickAttribLast();
                tickAttribLast.pastLimit(mask.get(0));
                tickAttribLast.unreported(mask.get(1));
                String exchange = readStr();
            ...
            case 3: // BidAsk
                ...
                com.ib.client.Decimal bidSize = readDecimal();
                com.ib.client.Decimal askSize = readDecimal();
    ...
    private void processCompletedOrderMsg() throws IOException {
        com.ib.client.Contract contract = new Contract();
        com.ib.client.Order order = new Order();
        OrderState orderState = new OrderState();
    ...
    public com.ib.client.Decimal readDecimal() throws IOException {
        String str = readStr();
        return (str == null || str.isEmpty() || 
                str.equals(String.valueOf(Long.MAX_VALUE)) ||
                str.equals(String.valueOf(Integer.MAX_VALUE)) ||
                str.equals(String.valueOf(Double.MAX_VALUE))) ? com.ib.client.Decimal.INVALID : Decimal.parse(str);
    }
