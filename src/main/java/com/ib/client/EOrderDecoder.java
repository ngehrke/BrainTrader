...
public class EOrderDecoder {
    ...
    private final com.ib.client.EDecoder m_eDecoder;

    private final com.ib.client.Contract m_contract;
    private final com.ib.client.Order m_order;
    private final com.ib.client.OrderState m_orderState;
    private final int m_version;
    private final int m_serverVersion;

    EOrderDecoder(com.ib.client.EDecoder eDecoder, Contract contract, Order order, OrderState orderState, int version, int serverVersion) {
        m_eDecoder = eDecoder;
        m_contract = contract;
        m_order = order;
    ...
    public void readFAParams() throws IOException {
        if ( m_version >= 7 ) {
            ...
            if ( m_serverVersion < com.ib.client.EClient.MIN_SERVER_VER_FA_PROFILE_DESUPPORT ) {
    ...
    public void readVolOrderParams(boolean readOpenOrderAttribs) throws IOException {
        if (m_version >= 11) {
            ...
            if (m_version == 11) {
                ...
                if (m_version >= 27 && !com.ib.client.Util.StringIsEmpty(m_order.getDeltaNeutralOrderType())) {
                ...
                if (m_version >= 31 && !com.ib.client.Util.StringIsEmpty(m_order.getDeltaNeutralOrderType())) {
                    if (readOpenOrderAttribs) {
        ...
        if (m_version >= 29) {
            ...
            if (comboLegsCount > 0) {
                ...
                for (int i = 0; i < comboLegsCount; ++i) {
                    ...
                    com.ib.client.ComboLeg comboLeg = new ComboLeg(conId, ratio, action, exchange, openClose,
        ...
        if (m_version >= 26) {
            ...
            if (smartComboRoutingParamsCount > 0) {
                ...
                for (int i = 0; i < smartComboRoutingParamsCount; ++i) {
                    com.ib.client.TagValue tagValue = new com.ib.client.TagValue();
    ...
    public void readHedgeParams() throws IOException {
        if (m_version >= 24) {
            ...
            if (!com.ib.client.Util.StringIsEmpty(m_order.getHedgeType())) {
    ...
    public void readAlgoParams() throws IOException {
        if (m_version >= 21) {
            ...
            if (!com.ib.client.Util.StringIsEmpty(m_order.getAlgoStrategy())) {
                int algoParamsCount = m_eDecoder.readInt();
                if (algoParamsCount > 0) {
                    for (int i = 0; i < algoParamsCount; ++i) {
                        com.ib.client.TagValue tagValue = new TagValue();
                        tagValue.m_tag = m_eDecoder.readStr();
                        tagValue.m_value = m_eDecoder.readStr();
    ...
    public void readWhatIfInfoAndCommission() throws IOException {
        if (m_version >= 16) {
            ...
            if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_WHAT_IF_EXT_FIELDS) {
    ...
    public void readPegToBenchParams() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PEGGED_TO_BENCHMARK) {
    ...
    public void readSoftDollarTier() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_SOFT_DOLLAR_TIER) {
    ...
    public void readAutoCancelParent() throws IOException {
        readAutoCancelParent(com.ib.client.EClient.MIN_VERSION);
    }
    ...
    public void readUsePriceMgmtAlgo() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PRICE_MGMT_ALGO) {
            m_order.usePriceMgmtAlgo(m_eDecoder.readBoolFromInt());
        }
    }
    
    public void readDuration() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_DURATION) {
            m_order.duration(m_eDecoder.readInt());
        }
    }

    public void readPostToAts() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_POST_TO_ATS) {
            m_order.postToAts(m_eDecoder.readIntMax());
        }
    }

    public void readPegBestPegMidOrderAttributes() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PEGBEST_PEGMID_OFFSETS) {
    ...
    public void readCustomerAccount() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_CUSTOMER_ACCOUNT) {
            m_order.customerAccount(m_eDecoder.readStr());
        }
    }

    public void readProfessionalCustomer() throws IOException {
        if (m_serverVersion >= com.ib.client.EClient.MIN_SERVER_VER_PROFESSIONAL_CUSTOMER) {
