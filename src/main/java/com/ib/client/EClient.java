...
public abstract class EClient {
    ...
    public synchronized void startAPI() {
        ...
        try {
        	com.ib.client.Builder b = prepareBuffer();
    ...
    public synchronized void placeOrder( int id, Contract contract, Order order) {
        ...
        try {
            final com.ib.client.Builder b = prepareBuffer();
           ...
           if (m_serverVersion >= 19) {
        	   ...
        	   double lower = (m_serverVersion == 26 && com.ib.client.Util.IsVolOrder(order.orderType()))
        	   		? Double.MAX_VALUE
        	   		: order.stockRangeLower();
        	   double upper = (m_serverVersion == 26 && com.ib.client.Util.IsVolOrder(order.orderType()))
   	   				? Double.MAX_VALUE
           ...
           if (m_serverVersion >= 26) { // Volatility orders
               ...
               if (m_serverVersion == 26) {
            	   // Volatility orders had specific watermark price attribs in server version 26
            	   double lower = com.ib.client.Util.IsVolOrder(order.orderType()) ? order.stockRangeLower() : Double.MAX_VALUE;
            	   double upper = com.ib.client.Util.IsVolOrder(order.orderType()) ? order.stockRangeUpper() : Double.MAX_VALUE;
                   b.sendMax( lower);
                   b.sendMax( upper);
               }
    ...
    public void requestFA( com.ib.client.Types.FADataType faDataType ) {
    ...
    protected abstract com.ib.client.Builder prepareBuffer();
    
    protected abstract void closeAndSend(com.ib.client.Builder buf) throws IOException;
    
    
    protected void validateInvalidSymbols(String host) throws EClientException {
        if (host != null && !com.ib.client.Builder.isAsciiPrintable(host)) {
            throw new EClientException(EClientErrors.INVALID_SYMBOL, host);
        }

        if (m_connectOptions != null && !com.ib.client.Builder.isAsciiPrintable(m_connectOptions)) {
            throw new EClientException(EClientErrors.INVALID_SYMBOL, m_connectOptions);
        }

        if (m_optionalCapabilities != null && !com.ib.client.Builder.isAsciiPrintable(m_optionalCapabilities)) {
    ...
    private void sendV100APIHeader() throws IOException {
    	try (com.ib.client.Builder builder = new com.ib.client.Builder(1024)) {
